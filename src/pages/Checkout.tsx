...
const Checkout = () => {
    ...
    try {
      if (user) {
        ...
        if (cartIds.length > 0) {
          ...
          if (error) console.error("Cart deletion error:", error);
        }
      } else {
  ...
  useEffect(() => {
    ...
    fetch(`https://viacep.com.br/ws/${clean}/json/`)
      .then(r => {
        if (!r.ok) throw new Error(`ViaCEP error: ${r.status}`);
        return r.json();
      })
      .then(data => {
        if (cancelled || data.erro) {
          console.warn("CEP not found or request cancelled");
          return;
        }
        setAddress(prev => ({
          ...prev,
  ...
  const saveDataAfterOrder = async () => {
    ...
    try {
      if (customerInfo.cpf) {
        const { error: cpfErr } = await supabase.from("profiles").update({ cpf: customerInfo.cpf } as any).eq("user_id", user.id);
        if (cpfErr) console.error("CPF save error:", cpfErr);
      }
      if (customerInfo.phone) {
        const { error: phoneErr } = await supabase.from("profiles").update({ phone: customerInfo.phone }).eq("user_id", user.id);
        if (phoneErr) console.error("Phone save error:", phoneErr);
      }
      if (address.street && address.number && address.zip) {
        ...
        if (!existing) {
          const { error: addrErr } = await supabase.from("saved_addresses").insert({
          ...
          if (addrErr) console.error("Address save error:", addrErr);
        }
      }
    } catch (e) {
  ...
  const createOrder = async (statusOverride?: string) => {
    try {
      const productIds = items.map((item: any) => (item.products as any)?.id).filter(Boolean);
      const { data: stockData, error: stockError } = await supabase.from("products").select("id, name, stock").in("id", productIds);

      if (stockError) throw new Error(`Erro ao verificar estoque: ${stockError.message}`);
      ...
      try {
        ...
        console.error("TikTok tracking error:", tiktokErr);
      }
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      if (paymentMethod === "whatsapp") {
        ...
        const whatsappUrl = `https://wa.me/5591936180774?text=${msg}`;
        window.open(whatsappUrl, '_blank');
        setStep("success");
        toast.success("Pedido criado! Clique no botão abaixo para finalizar via WhatsApp. 🎉");
        return;
      }
