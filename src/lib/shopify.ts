...
const SHOPIFY_STORE_PERMANENT_DOMAIN = "elle-make-loja-virtual-qw73k.myshopify.com";
const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
const SHOPIFY_STOREFRONT_TOKEN = "58422dbd1e979bcdcdc41338ea5704ce";
...
export async function storefrontApiRequest(query: string, variables: Record<string, unknown> = {}) {
  ...
  if (response.status === 402) {
    toast.error("Shopify: Pagamento necessário", {
      description: "A API Shopify requer um plano ativo. Visite admin.shopify.com para atualizar.",
    });
    return;
  }
  ...
  if (data.errors) {
    throw new Error(`Shopify API: ${data.errors.map((e: { message: string }) => e.message).join(", ")}`);
  }
  return data;
}
